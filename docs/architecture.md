...
3. accepts a unique conservative extended-name match when an existing sufficiently specific organization name is the full prefix and only a short non-legal suffix was added;
...
For example, an extracted regional expansion such as `Landesamt für Besoldung und Versorgung Baden-Württemberg` can resolve to the existing canonical `Landesamt für Besoldung und Versorgung`, while adding a different corporate legal form is not collapsed automatically.
