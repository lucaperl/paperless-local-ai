...
The classification request extracts the actual sender/issuer as a short free-text name. Local resolution can apply a normalized exact match, a deliberately strong unambiguous fuzzy match or a conservative unambiguous extended-name match. Other plausible names are exposed through Paperless Document Suggestions; unreliable output is left empty.
