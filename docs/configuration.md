...
The classification request extracts the actual sender/issuer as a short free-text name. Local resolution first checks for a unique normalized exact match. If there is no exact match, fuzzy resolution uses two versioned **App Settings → Matching** controls:
