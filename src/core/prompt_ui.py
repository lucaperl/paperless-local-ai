...
from app_config import (
    ...
    validate_correspondent_matching,
)
from correspondent_resolver import resolve_correspondent, simulate_correspondent_match
from history_client import history_context_for_document, refresh_history
from history_common import (
    cached_history_state,
    history_excluded_tag_names,
    history_matching_settings,
)
from paperless_ui_state import integration_package_ready, sync_projection
  ...
  <div class="section-grid">
    <div class="card section"><h2>Pipeline</h2><p>Paperless remains the document system of record. paperless-local-ai improves scanned-page OCR, then applies local metadata automation.</p>
      <div class="flow">
        ...
        <div class="flow-row local"><div class="flow-dot">4</div><div class="flow-copy"><div class="flow-title">Tag routing</div><div class="flow-desc">Hybrid tagging first checks reviewed Paperless documents for a high-confidence complete tag-set match. Otherwise the LLM decides tags; LLM direct always lets the model decide.</div></div></div>
...
<section class="page" id="page-classification">
  ...
  <div class="tab-page active" id="class-test">
    ...
    <div class="result">
      ...
      <div class="card panel"><h3>Tagging route</h3><p class="mini">Explains whether the tag set came from reviewed history or from the LLM.</p><pre id="taggingPreview" class="preview">Run Preview prompts or Run model test to populate this section.</pre></div>
  ...
  <div class="tab-page" id="class-tagging">
    ...
    <div class="card panel"><h3>Tagging strategy</h3><p class="mini">Choose how content tags are decided. Title, document type, date and sender still use the configured LLM.</p>
      <div class="strategy-grid">
        <label class="strategy-card"><div class="strategy-head"><div class="strategy-title"><input type="radio" name="taggingMode" value="history_assisted"><span>Hybrid tagging</span></div><span class="badge-rec">Recommended for small models</span></div><div class="strategy-copy">Compares documents with reviewed examples and reuses a complete known leaf-tag set only when similarity and neighbor agreement are strong. Otherwise the LLM decides using Tag Guidance and relevant examples.</div><div class="strategy-best"><strong>Best fit:</strong> compact local models, including the 4B reference model. · <a href="__TAGGING_DOCS_URL__#hybrid-tagging" target="_blank" rel="noreferrer">How Hybrid tagging works</a></div></label>
    ...
    <div class="card panel"><div style="display:flex;align-items:flex-start;justify-content:space-between;gap:12px;flex-wrap:wrap"><div><h3 style="margin-bottom:4px">History health</h3><p class="mini" style="margin:0">Built from documents whose configured review tag has been removed after human review. Documents still carrying the classification queue or error tag are excluded. The index checks for changes at most every five minutes when used.</p></div><button id="taggingRefreshBtn" class="btn">Refresh reviewed history</button></div><div id="historyState" class="status-box" style="margin-top:12px">Loading history status…</div>
      ...
      <details class="section-help" style="margin-top:14px"><summary>Advanced History matching</summary><div class="help-body"><p style="margin-top:0">Controls when Hybrid tagging may reuse a complete reviewed leaf-tag set. Lower values increase automatic reuse but also increase the risk of a wrong tag set. <a href="__TAGGING_DOCS_URL__#advanced-history-matching" target="_blank" rel="noreferrer">Detailed behavior ↗</a></p><div class="form-grid three"><div class="field"><label>Minimum similarity</label><input id="historyMatchSimilarity" type="number" min="0.5" max="1" step="0.01"><div class="field-help">The closest reviewed document must reach this similarity before its complete tag set can be reused.</div></div><div class="field"><label>Minimum support</label><input id="historyMinSupport" type="number" min="2" max="5" step="1"><div class="field-help">How many of the five nearest reviewed documents must use the winning complete tag set.</div></div><div class="field"><label>Minimum winner share</label><input id="historyMinWinnerShare" type="number" min="0.5" max="1" step="0.01"><div class="field-help">Required similarity-weighted vote share for the winning complete tag set.</div></div></div><div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;margin-top:12px"><button id="historyMatchingSaveBtn" class="btn">Save History matching</button><div id="historyMatchingSaveStatus" class="status-box" style="flex:1">Saved values loaded.</div></div></div></details>
...
<section class="page" id="page-app-settings">
  ...
  <div class="tabs" data-tabs="app"><button class="tab active" data-tab="app-connections">Connections</button><button class="tab" data-tab="app-workflow">Pipeline &amp; Tags</button><button class="tab" data-tab="app-matching">Matching</button><button class="tab" data-tab="app-ocr">OCR</button><button class="tab" data-tab="app-runtime">Runtime</button><button class="tab" data-tab="app-history">History</button></div>

  <div class="tab-page active" id="app-connections"><details class="section-help"><summary>About connections</summary><div class="help-body">URLs must be reachable from the app containers. The Paperless token is a deployment secret and is never shown or stored in versioned UI configuration.</div></details><div class="connection-row"><div class="card connection"><h3>Paperless-ngx</h3><div class="field"><label>Paperless URL</label><input id="appPaperlessUrl"></div><div class="field" style="margin-top:12px"><label>API token</label><div id="appTokenStatus" class="status-box">Loading…</div></div></div><div class="card connection"><h3>Ollama</h3><div class="field"><label>Ollama URL</label><input id="appOllamaUrl"></div></div></div><div class="card panel" style="margin-top:14px"><div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap"><button id="appConnectionTestBtn" class="btn good">Test connections with current draft</button><div id="appConnectionStatus" class="status-box" style="flex:1">Not tested yet.</div></div></div><div class="card panel" style="margin-top:14px"><h3>Paperless shortcut</h3><p class="mini">Adds a <strong>paperless-local-ai</strong> button to the Paperless Settings header. Requires two environment variables in Paperless. <a href="__PAPERLESS_SETUP_DOCS_URL__#optional-paperless-settings-shortcut" target="_blank" rel="noreferrer">Setup guide ↗</a></p><div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap"><button id="appPaperlessUiToggleBtn" class="btn">Enable shortcut</button><button id="appPaperlessUiRecheckBtn" class="btn">Recheck setup</button><div id="appPaperlessUiStatus" class="status-box" style="flex:1">Checking Paperless setup…</div></div></div></div>
  ...
  <div class="tab-page" id="app-matching">
    <div class="action-note"><span>i</span><div><strong>Exact matches stay exact.</strong> The sender extracted by the LLM is normalized first. A unique normalized exact match is applied without fuzzy thresholds. The settings below are used only when no exact match exists.</div></div>
    <div class="card panel"><h3>Correspondent matching</h3><p class="mini">A fuzzy match is applied automatically only when <strong>both</strong> conditions pass. The default uses 91% minimum similarity with the existing 4 pp ambiguity margin.</p><div class="form-grid"><div class="field"><label for="appCorrespondentMinSimilarity">Minimum similarity (%)</label><input id="appCorrespondentMinSimilarity" type="number" min="0" max="100" step="0.5" inputmode="decimal"><div class="field-help">How similar the extracted sender must be to the best existing Paperless correspondent. Lower = more permissive and accepts more name variation. Higher = more conservative and reduces the risk of matching different senders.</div></div><div class="field"><label for="appCorrespondentMinMargin">Minimum winner margin (percentage points)</label><input id="appCorrespondentMinMargin" type="number" min="0" max="100" step="0.5" inputmode="decimal"><div class="field-help">How much better the best candidate must score than the second-best candidate. This blocks ambiguous cases even when the best score itself is high. Lower = accepts more ambiguity. Higher = requires a clearer winner.</div></div></div><div id="appCorrespondentMatchingSummary" class="status-box" style="margin-top:12px" aria-live="polite"></div><div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;margin-top:10px"><button id="appCorrespondentResetBtn" class="btn" type="button">Reset to defaults (91% / 4 pp)</button><span class="mini">These values are saved with App Settings and used by the next classification job.</span></div></div>
    <details class="section-help" style="margin-top:14px"><summary>Test correspondent matching</summary><div class="help-body"><p style="margin-top:0">Try an extracted sender name against the <strong>current Paperless correspondents</strong> using the unsaved values above. The test is read-only: it does not call Ollama, create a correspondent, or modify a Paperless document.</p><div class="test-row" style="grid-template-columns:minmax(260px,1fr) auto"><div class="field"><label for="correspondentTestCandidate">Extracted sender</label><input id="correspondentTestCandidate" maxlength="255" autocomplete="off" placeholder="e.g. Musterwerke Energi GmbH"></div><button id="correspondentTestBtn" class="btn" type="button">Test match</button></div><div id="correspondentTestStatus" class="status-box" style="margin-top:12px" aria-live="polite">Enter a sender name to see the three closest current correspondents and the exact decision.</div><div id="correspondentTestResult" style="margin-top:12px" aria-live="polite"></div></div></details>
  </div>
...
<section class="page" id="page-document-chat">
  ...
  <div class="card panel">
    ...
    <details class="advanced-panel">
      <summary>Advanced prompt assembly</summary>
      <div class="advanced-body">
        <p class="mini" style="margin-top:0">Controls how much previous conversation the answer model receives and the final user-message wrapper built around retrieved excerpts. These settings do not change embeddings or require an index rebuild.</p>
        <div class="form-grid">
          <div class="field"><label for="ragConversationHistory">Conversation history messages</label><input id="ragConversationHistory" type="number" min="0" max="24" step="1"><div class="field-help">Maximum number of previous user and assistant messages sent to the answer model. <strong>0</strong> makes each answer independent of earlier conversation text. Retrieval history is configured separately below.</div></div>
        </div>
        <div class="field" style="margin-top:14px">
          <label for="ragSourceTemplate">Retrieved source template <span class="info-dot" title="Rendered once for every primary retrieved source before all source blocks are inserted into the answer prompt.">i</span></label>
          <textarea id="ragSourceTemplate" class="mono compact-textarea"></textarea>
          <div class="field-help">Controls the metadata and text sent for each retrieved source. The default includes title, created date, correspondent, document type and document ID. Metadata is read live from Paperless for each chat, so changing this template does <strong>not</strong> require an index rebuild. Variables such as full content or raw JSON can be very large and are intended for expert use.</div>
          <div class="template-actions"><button id="ragSourcePreviewBtn" class="btn" type="button">Preview</button><button id="ragResetSourceTemplateBtn" class="btn" type="button">Reset</button></div>
          <details class="mini-details"><summary>Variables</summary><div class="help-body"><div id="ragSourcePlaceholderGrid" class="placeholder-grid"></div></div></details>
          <pre id="ragSourcePreview" class="preview compact-preview" style="display:none"></pre>
        </div>

        <div class="field" style="margin-top:14px">
          <label for="ragAnswerTemplate">Answer prompt template <span class="info-dot" title="This is the complete final user-message wrapper around retrieved excerpts and the current question.">i</span></label>
          <textarea id="ragAnswerTemplate" class="mono compact-textarea"></textarea>
          <div class="field-help">Controls exactly where retrieved document excerpts and the current question appear in the final user message sent to the chat model.</div>
          <div class="template-actions"><button id="ragAnswerPreviewBtn" class="btn" type="button">Preview</button><button id="ragResetAnswerTemplateBtn" class="btn" type="button">Reset</button></div>
          <details class="mini-details"><summary>Variables</summary><div class="help-body"><div id="ragAnswerPlaceholderGrid" class="placeholder-grid"></div></div></details>
          <pre id="ragAnswerPreview" class="preview compact-preview" style="display:none"></pre>
        </div>
      </div>
    </details>
  ...
  <div class="card panel">
    ...
    <details class="advanced-panel">
      ...
      <div class="advanced-body">
        ...
        <div class="form-grid three">
          <div class="field"><label for="ragHistoryMode">Retrieval history</label><select id="ragHistoryMode"><option value="user_only">User messages only</option><option value="user_and_assistant">User + assistant messages</option></select><div class="field-help">Choose which roles from the selected previous turns are included in the embedding query. User only is safer against feeding a mistaken model answer back into retrieval.</div></div>
          <div class="field"><label for="ragHistoryTurns">Previous user turns</label><input id="ragHistoryTurns" type="number" min="0" max="8" step="1" value="3"><div class="field-help">Number of earlier user turns that define the retrieval history window. <strong>Default: 3.</strong> In User + assistant mode, assistant replies inside those turns are included too. <strong>0</strong> uses only the current question.</div></div>
          ...
          <div class="field"><label for="ragMaxChunksPerDoc">Max chunks per document</label><input id="ragMaxChunksPerDoc" type="number" min="1" max="64" step="1" placeholder="Unlimited"><div class="field-help">Limits how many primary similarity hits may come from one document. Leave blank for no limit; lower values encourage source diversity.</div></div>
          <div class="field"><label for="ragAdjacentChunks">Adjacent chunks</label><select id="ragAdjacentChunks"><option value="0">Off</option><option value="1">±1 chunk</option><option value="2">±2 chunks</option><option value="3">±3 chunks</option></select><div class="field-help">Adds neighboring chunks around each primary similarity hit before prompt assembly. This can restore context across chunk boundaries without another model call. It does not change Top-K ranking.</div></div>
          <div class="field"><label for="ragRetrievalContextPercent">Document context budget (%)</label><input id="ragRetrievalContextPercent" type="number" min="10" max="100" step="5" placeholder="Auto"><div class="field-help">Optional cap on how much of the approximate input context document excerpts may use. Leave blank for automatic use of the remaining prompt budget.</div></div>
          <div class="field"><label for="ragRetrievalDiagnostics">Retrieval diagnostics by default</label><select id="ragRetrievalDiagnostics"><option value="false">Off</option><option value="true">On</option></select><div class="field-help">When enabled, answers can show the effective embedding query, similarity scores, selected chunk ordinals and prompt-budget details. No extra model request is made. This can be overridden per chat.</div></div>
        </div>
      </div>
    </details>
  </div>
...
let currentRagConfig=null,currentRagState=null,ragDefaultSystemPrompt="",ragDefaultQueryTemplate="",ragDefaultDocumentTemplate="",ragDefaultSourceTemplate="",ragDefaultAnswerTemplate="";
let ragQueryPlaceholders=[],ragDocumentPlaceholders=[],ragSourcePlaceholders=[],ragAnswerPlaceholders=[];
...
function markUnsaved(id='saveStatus'){const el=$(id);if(el){el.textContent='Unsaved changes.';el.className='toolbar-status warn-text'}}function markHistoryMatchingUnsaved(){const el=$('historyMatchingSaveStatus');if(el){el.textContent='Unsaved changes.';el.className='status-box warn'}}
...
function historyMatchingDraft(){const c=JSON.parse(JSON.stringify(currentAppConfig||{}));c.history={match_similarity:Number($('historyMatchSimilarity').value),min_support:Number($('historyMinSupport').value),min_winner_share:Number($('historyMinWinnerShare').value)};return c}
function correspondentMatchingDraft(){const similarity=Number($('appCorrespondentMinSimilarity').value)/100,margin=Number($('appCorrespondentMinMargin').value)/100;if(!Number.isFinite(similarity)||similarity<0||similarity>1)throw new Error('Minimum correspondent similarity must be between 0% and 100%.');if(!Number.isFinite(margin)||margin<0||margin>1)throw new Error('Minimum winner margin must be between 0 and 100 percentage points.');return {minimum_similarity:similarity,minimum_margin:margin}}
function compactNumber(v,digits=2){const n=Number(v);if(!Number.isFinite(n))return'—';return n.toFixed(digits).replace(/\.?0+$/,'')}
function pct(v){return Number.isFinite(Number(v))?`${compactNumber(Number(v)*100)}%`:'—'}function pp(v){return Number.isFinite(Number(v))?`${compactNumber(Number(v)*100)} pp`:'—'}
function updateCorrespondentMatchingSummary(){const el=$('appCorrespondentMatchingSummary');if(!el)return;try{const m=correspondentMatchingDraft();el.innerHTML=`<strong>Automatic fuzzy matching currently requires both:</strong> best similarity ≥ <strong>${pct(m.minimum_similarity)}</strong> <em>and</em> the best candidate must be ≥ <strong>${pp(m.minimum_margin)}</strong> ahead of the second-best candidate. <span class="mini">Unique normalized exact matches bypass these thresholds.</span>`;el.className='status-box'}catch(e){el.textContent=e.message;el.className='status-box bad'}}
function markCorrespondentTestStale(){const el=$('correspondentTestStatus');if(el&&$('correspondentTestResult')?.innerHTML){el.textContent='Matching settings changed · run the test again to refresh this result.';el.className='status-box warn'}}
function renderCorrespondentMatchingResult(payload){const s=payload?.simulation||{},r=s.resolution||{},rows=Array.isArray(s.candidates)?s.candidates:[],el=$('correspondentTestResult');const labels={existing_exact:'Exact normalized match',existing_fuzzy:'Automatic fuzzy match',new_suggestion:'No automatic match',empty:'No usable sender name'};const good=r.status==='existing_exact'||r.status==='existing_fuzzy';const title=labels[r.status]||r.status||'Result';const selected=r.resolved?`<div class="mini" style="margin-top:5px">→ <strong>${esc(r.resolved)}</strong> would be selected.</div>`:r.status==='empty'?'<div class="mini" style="margin-top:5px">The input is not treated as a reliable sender candidate.</div>':'<div class="mini" style="margin-top:5px">→ No existing correspondent would be selected automatically.</div>';const candidates=rows.length?`<table class="tag-table"><thead><tr><th>#</th><th>Current Paperless correspondent</th><th>Similarity</th></tr></thead><tbody>${rows.map((x,i)=>`<tr><td>${i+1}</td><td>${esc(x.name)}${i===0?' <span class="mini">· best</span>':''}</td><td>${pct(x.score)}</td></tr>`).join('')}</tbody></table>`:'<div class="mini">No current correspondent candidates were scored.</div>';let checks='';if(r.status==='existing_exact'){checks='<div class="status-box good" style="margin-top:10px">Normalized names are identical. Fuzzy similarity and winner-margin thresholds are not used for this match.</div>'}else if(r.status!=='empty'&&rows.length){const simOk=s.similarity_pass===true,marginOk=s.margin_pass===true;const marginText=s.winner_margin==null?'No second-best candidate; the margin gate is non-restrictive.':`${pp(s.winner_margin)} ${s.winner_margin>=s.minimum_margin?'≥':'<'} ${pp(s.minimum_margin)}`;checks=`<div class="result-fields" style="margin-top:10px"><div class="result-field"><span>Minimum similarity</span><strong class="${simOk?'good-text':'bad-text'}">${pct(rows[0].score)} ${rows[0].score>=s.minimum_similarity?'≥':'<'} ${pct(s.minimum_similarity)} · ${simOk?'Pass':'Fail'}</strong></div><div class="result-field"><span>Winner margin</span><strong class="${marginOk?'good-text':'bad-text'}">${marginText} · ${marginOk?'Pass':'Fail'}</strong></div></div>`}el.innerHTML=`<div class="result-summary"><div class="result-state ${good?'good-text':r.status==='empty'?'warn-text':'warn-text'}">${esc(title)}</div><div class="mini">Tested against ${Number(s.existing_count||0)} current Paperless correspondents.</div>${selected}${checks}</div><div style="margin-top:10px"><strong>Closest candidates</strong>${candidates}</div>`}
async function testCorrespondentMatching(){const candidate=$('correspondentTestCandidate').value.trim();if(!candidate){const el=$('correspondentTestStatus');el.textContent='Enter an extracted sender name first.';el.className='status-box bad';return}let matching;try{matching=correspondentMatchingDraft()}catch(e){const el=$('correspondentTestStatus');el.textContent=e.message;el.className='status-box bad';return}const status=$('correspondentTestStatus');status.textContent='Testing against current Paperless correspondents…';status.className='status-box';try{const r=await api('/api/app/correspondent-matching/test',{method:'POST',body:JSON.stringify({candidate,matching})});renderCorrespondentMatchingResult(r);status.textContent='Read-only test complete · nothing was changed in Paperless.';status.className='status-box good'}catch(e){status.textContent=e.message;status.className='status-box bad'}}
...
function renderPaperlessUi(c){const s=c?.paperless_ui||{enabled:false,control_center_url:''},btn=$('appPaperlessUiToggleBtn'),status=$('appPaperlessUiStatus');btn.textContent=s.enabled?'Disable Paperless UI':'Enable Paperless UI';const setupOk=paperlessUiSetup?.ok===true;btn.disabled=!s.enabled&&!setupOk;btn.className='btn '+(!s.enabled&&setupOk?'good':'');if(paperlessUiSetup===null){status.textContent='Checking Paperless setup…';status.className='status-box';return}if(paperlessUiSetup.package_ready===false||!paperlessUiReady){status.textContent='Integration files are not published yet.';status.className='status-box warn';return}if(!setupOk){status.textContent=paperlessUiSetup.reachable===true?'Setup required · Add the two required environment variables to Paperless and restart it.':`Could not verify Paperless integration · ${paperlessUiSetup.detail||'Paperless is unavailable.'}`;status.className='status-box warn';return}status.textContent=s.enabled?`Enabled · Paperless integration verified · opens ${s.control_center_url}`:'Ready · Paperless integration verified.';status.className='status-box good'}
function appFill(c,tokenConfigured,ready=null){currentAppConfig=c;if(typeof ready==='boolean')paperlessUiReady=ready;$('appPaperlessUrl').value=c.connections.paperless_url;$('appOllamaUrl').value=c.connections.ollama_url;$('appLlmQueueTag').value=c.workflow.llm_queue_tag;$('appLlmErrorTag').value=c.workflow.llm_error_tag;$('appReviewTag').value=c.workflow.review_tag;$('appExtraExcludedTags').value=(c.workflow.extra_excluded_tags||[]).join(', ');$('appOcrLanguage').value=c.ocr.language;$('appOcrVersion').value=c.ocr.version;$('appOcrModelProfile').value=c.ocr.model_profile||'medium';$('appOcrMaxSidePixels').value=c.ocr.max_side_pixels||3000;$('appOcrRetryDelays').value=(c.ocr.retry_delays_seconds||[]).join(', ');$('appOcrDevice').value=c.ocr.device;$('appPollInterval').value=c.runtime.poll_interval_seconds;$('appReviewPruneInterval').value=c.runtime.review_prune_interval_seconds;$('appDryRun').value=String(c.runtime.dry_run);const h=c.history||{match_similarity:0.62,min_support:2,min_winner_share:0.50};$('historyMatchSimilarity').value=h.match_similarity;$('historyMinSupport').value=h.min_support;$('historyMinWinnerShare').value=h.min_winner_share;const m=c.correspondent_matching||{minimum_similarity:0.91,minimum_margin:0.04};$('appCorrespondentMinSimilarity').value=compactNumber(Number(m.minimum_similarity)*100);$('appCorrespondentMinMargin').value=compactNumber(Number(m.minimum_margin)*100);updateCorrespondentMatchingSummary();$('historyMatchingSaveStatus').textContent='Saved values loaded.';$('historyMatchingSaveStatus').className='status-box good';$('appConfigStatus').textContent=`App settings v${c.version} · ${c.runtime.dry_run?'metadata dry run':'metadata writes enabled'}`;$('appTokenStatus').textContent=tokenConfigured?'API token is configured in the deployment environment.':'API token is missing from the deployment environment.';$('appTokenStatus').className='status-box '+(tokenConfigured?'good':'bad');renderPaperlessUi(c);updateOverview()}
...
function renderHistoryHealth(h){currentHistoryStatus=h;const state=$('historyState');const strategy=selectedTaggingMode();const strategyNote=strategy==='llm_only'?'History is not used by the current LLM direct strategy. ':'';const cacheState=h.cache_state||'ready';if(h.status==='Error'){state.textContent=`History could not be refreshed: ${h.last_error||'unknown error'}`;state.className='status-box bad'}else if(cacheState==='missing'){state.textContent=strategyNote+'History cache has not been built yet. It will build automatically on the first Hybrid classification or when you refresh reviewed history.';state.className='status-box warn'}else if(cacheState==='corrupt'){state.textContent=strategyNote+'The cached history index could not be validated. It will rebuild automatically on the next Hybrid classification or when you refresh reviewed history.';state.className='status-box warn'}else if(h.stale){state.textContent=strategyNote+'History settings or reviewed documents have changed since the cached index was built. It will refresh automatically on the next Hybrid classification or when you refresh it here.';state.className='status-box warn'}else{state.textContent=strategyNote+(h.status==='Ready'?'History index is ready. Strong evidence can be reused by Hybrid tagging.':'There is not enough reviewed history yet. Hybrid tagging will use the LLM fallback.');state.className='status-box '+(h.status==='Ready'?'good':'')}
$('historyDocs').textContent=h.source?.reviewed_documents??h.reviewed_documents??0;$('historyTags').textContent=`${h.tags_represented??0} / ${h.eligible_tags??0}`;const sample=h.estimated_reuse_sample_size||0;$('historyReuse').textContent=sample?`${h.estimated_reuse_percent}%`:'—';$('historyIssues').textContent=h.potential_inconsistency_count??0;$('historyUpdated').textContent=`Last updated: ${h.last_updated?new Date(h.last_updated).toLocaleString():'not built'}${sample&&sample!==(h.reviewed_documents||0)?` · reuse estimate sampled from ${sample} documents`:''}`;
...
async function checkPaperlessUiSetup(){paperlessUiSetup=null;renderPaperlessUi(currentAppConfig);try{paperlessUiSetup=await api('/api/app/paperless-ui/status')}catch(e){paperlessUiSetup={ok:false,reachable:false,package_ready:paperlessUiReady,detail:e.message}}renderPaperlessUi(currentAppConfig)}
...
async function loadApp(){try{const r=await api('/api/app/state');appFill(r.config,r.token_configured,r.paperless_ui_integration_ready);renderAppHistory(r.history||[]);checkConnections(r.config);await checkPaperlessUiSetup()}catch(e){setStatus('appSaveStatus',e.message,false)}}
...
for(const id of ['appPaperlessUrl','appOllamaUrl','appLlmQueueTag','appLlmErrorTag','appReviewTag','appExtraExcludedTags','appOcrLanguage','appOcrModelProfile','appOcrMaxSidePixels','appOcrRetryDelays','appOcrDevice','appPollInterval','appReviewPruneInterval','appDryRun']){$(id)?.addEventListener('input',()=>markUnsaved('appSaveStatus'));$(id)?.addEventListener('change',()=>markUnsaved('appSaveStatus'))}for(const id of ['appCorrespondentMinSimilarity','appCorrespondentMinMargin']){$(id)?.addEventListener('input',()=>{markUnsaved('appSaveStatus');updateCorrespondentMatchingSummary();markCorrespondentTestStale()});$(id)?.addEventListener('change',()=>{markUnsaved('appSaveStatus');updateCorrespondentMatchingSummary();markCorrespondentTestStale()})}$('appCorrespondentResetBtn').onclick=()=>{$('appCorrespondentMinSimilarity').value='91';$('appCorrespondentMinMargin').value='4';markUnsaved('appSaveStatus');updateCorrespondentMatchingSummary();markCorrespondentTestStale()};$('correspondentTestBtn').onclick=testCorrespondentMatching;$('correspondentTestCandidate').addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();testCorrespondentMatching()}});
...
function ragDraft(){return {
  embedding_model:$('ragEmbeddingModel').value.trim(),embedding_query_template:$('ragQueryTemplate').value,document_embedding_template:$('ragDocumentTemplate').value,source_prompt_template:$('ragSourceTemplate').value,answer_prompt_template:$('ragAnswerTemplate').value,
  ...
  retrieval_history_mode:$('ragHistoryMode').value,retrieval_history_turns:Number($('ragHistoryTurns').value),retrieval_min_similarity:optionalNumber('ragMinSimilarity'),max_chunks_per_document:optionalNumber('ragMaxChunksPerDoc'),adjacent_chunks:Number($('ragAdjacentChunks').value),retrieval_context_percent:optionalNumber('ragRetrievalContextPercent'),system_prompt:$('ragSystemPrompt').value,timezone:$('ragTimezone').value.trim(),
  chat_defaults:{model:$('ragChatModel').value.trim(),think:$('ragThink').value,num_ctx:Number($('ragNumCtx').value),top_k:Number($('ragRetrievalTopK').value),temperature:Number($('ragTemperature').value),num_predict:Number($('ragNumPredict').value),conversation_history_messages:Number($('ragConversationHistory').value),show_retrieval_diagnostics:$('ragRetrievalDiagnostics').value==='true',sampler_top_k:optionalNumber('ragSamplerTopK'),top_p:optionalNumber('ragTopP'),min_p:optionalNumber('ragMinP'),repeat_penalty:optionalNumber('ragRepeatPenalty'),repeat_last_n:optionalNumber('ragRepeatLastN'),seed:optionalNumber('ragSeed'),stop:stopDraft()}
}}
...
function renderEmbeddingPlaceholders(){renderPlaceholderGrid('ragQueryPlaceholderGrid',ragQueryPlaceholders,{RETRIEVAL_QUERY:'Current question plus the configured retrieval history',CURRENT_QUESTION:'Current question only',PREVIOUS_USER_CONTEXT:'Previous user turns in the selected retrieval window',PREVIOUS_HISTORY_CONTEXT:'Effective previous context using the selected retrieval-history mode',SEARCH_SCOPE:'Current search scope'});renderPlaceholderGrid('ragDocumentPlaceholderGrid',ragDocumentPlaceholders,{CHUNK:'Raw indexed text chunk',DOCUMENT_TITLE:'Paperless document title',DOCUMENT_CREATED:'Paperless created date',DOCUMENT_ID:'Paperless document ID'})}
function renderSourcePlaceholders(){renderPlaceholderGrid('ragSourcePlaceholderGrid',ragSourcePlaceholders,{SOURCE_NUMBER:'1-based source number used by citations',SIMILARITY_SCORE:'Similarity score of the primary retrieval hit',CHUNK_ORDINAL:'Primary chunk ordinal inside the document',NEIGHBOR_ORDINALS:'Adjacent chunk ordinals included as extra context',CHUNK:'Retrieved chunk text including configured neighbors',DOCUMENT_ID:'Paperless document ID',DOCUMENT_TITLE:'Current Paperless title',DOCUMENT_CONTENT:'Full Paperless OCR/content text; usually too large for normal source templates',DOCUMENT_CORRESPONDENT:'Resolved correspondent name',DOCUMENT_CORRESPONDENT_ID:'Correspondent ID',DOCUMENT_TYPE:'Resolved document type name',DOCUMENT_TYPE_ID:'Document type ID',DOCUMENT_STORAGE_PATH:'Resolved storage path name',DOCUMENT_STORAGE_PATH_ID:'Storage path ID',DOCUMENT_TAGS:'Comma-separated resolved tag names',DOCUMENT_TAG_IDS:'Comma-separated tag IDs',DOCUMENT_CREATED:'Paperless created date',DOCUMENT_CREATED_DATE:'Deprecated created_date field when returned',DOCUMENT_MODIFIED:'Paperless modified timestamp',DOCUMENT_ADDED:'Paperless added timestamp',DOCUMENT_DELETED_AT:'Deleted timestamp when present',DOCUMENT_ARCHIVE_SERIAL_NUMBER:'Archive serial number (ASN)',DOCUMENT_ORIGINAL_FILE_NAME:'Original file name',DOCUMENT_ARCHIVED_FILE_NAME:'Archived file name',DOCUMENT_DUPLICATE_DOCUMENTS:'Duplicate documents as JSON',DOCUMENT_OWNER:'Resolved owner username/name',DOCUMENT_OWNER_ID:'Owner ID',DOCUMENT_PERMISSIONS:'Permissions as JSON when returned',DOCUMENT_USER_CAN_CHANGE:'Whether the current Paperless user may change the document',DOCUMENT_IS_SHARED_BY_REQUESTER:'Whether it is shared by the requester',DOCUMENT_NOTES:'Notes as JSON',DOCUMENT_CUSTOM_FIELDS:'Custom fields as JSON with resolved field names when available',DOCUMENT_CUSTOM_FIELDS_RAW:'Raw custom_fields JSON',DOCUMENT_PAGE_COUNT:'Page count',DOCUMENT_MIME_TYPE:'MIME type',DOCUMENT_ROOT_DOCUMENT:'Raw root_document value',DOCUMENT_ROOT_DOCUMENT_ID:'Root document ID',DOCUMENT_VERSIONS:'Document versions as JSON',DOCUMENT_METADATA_JSON:'All returned document metadata except full content, as JSON',DOCUMENT_RAW_JSON:'Complete Paperless document API response, including content'})}

function renderAnswerPlaceholders(){renderPlaceholderGrid('ragAnswerPlaceholderGrid',ragAnswerPlaceholders,{DOCUMENT_EXCERPTS:'Retrieved source blocks selected for the answer',QUESTION:'Current user question',SEARCH_SCOPE:'Current search scope',SOURCE_COUNT:'Number of source blocks included',CURRENT_DOCUMENT_ID:'Current document ID when document scope is active'})}
...
function previewQueryTemplate(){const question='What is the notice period in my service contract?';const previousUser='Find the relevant service agreement.';const previousAssistant='A candidate agreement was identified.';const includeAssistant=$('ragHistoryMode').value==='user_and_assistant';const previousHistory=includeAssistant?`User: ${previousUser}\nAssistant: ${previousAssistant}`:previousUser;const label=includeAssistant?'Previous conversation context:':'Previous user context:';const retrieval=`${label}\n${previousHistory}\nCurrent question:\n${question}`;$('ragQueryPreview').textContent=renderTemplate($('ragQueryTemplate').value,{RETRIEVAL_QUERY:retrieval,CURRENT_QUESTION:question,PREVIOUS_USER_CONTEXT:previousUser,PREVIOUS_HISTORY_CONTEXT:previousHistory,SEARCH_SCOPE:'all documents'});$('ragQueryPreview').style.display='block'}
...
function previewSourceTemplate(){const values={SOURCE_NUMBER:'1',SIMILARITY_SCORE:'0.912345',CHUNK_ORDINAL:'3',NEIGHBOR_ORDINALS:'2, 4',CHUNK:'This is a synthetic retrieved passage used only for template preview.',DOCUMENT_ID:'42',DOCUMENT_TITLE:'Sample Service Agreement',DOCUMENT_CONTENT:'Synthetic full document content for preview.',DOCUMENT_CORRESPONDENT:'Example Organization',DOCUMENT_CORRESPONDENT_ID:'7',DOCUMENT_TYPE:'Agreement',DOCUMENT_TYPE_ID:'3',DOCUMENT_STORAGE_PATH:'Contracts',DOCUMENT_STORAGE_PATH_ID:'2',DOCUMENT_TAGS:'Finance, Review',DOCUMENT_TAG_IDS:'5, 8',DOCUMENT_CREATED:'2026-01-15',DOCUMENT_CREATED_DATE:'2026-01-15',DOCUMENT_MODIFIED:'2026-02-01T12:30:00+00:00',DOCUMENT_ADDED:'2026-01-15T09:00:00+00:00',DOCUMENT_DELETED_AT:'',DOCUMENT_ARCHIVE_SERIAL_NUMBER:'123',DOCUMENT_ORIGINAL_FILE_NAME:'sample.pdf',DOCUMENT_ARCHIVED_FILE_NAME:'2026/sample.pdf',DOCUMENT_DUPLICATE_DOCUMENTS:'[]',DOCUMENT_OWNER:'sample-user',DOCUMENT_OWNER_ID:'1',DOCUMENT_PERMISSIONS:'{}',DOCUMENT_USER_CAN_CHANGE:'true',DOCUMENT_IS_SHARED_BY_REQUESTER:'false',DOCUMENT_NOTES:'[]',DOCUMENT_CUSTOM_FIELDS:'[{"field":4,"field_name":"Reference","value":"ABC-123"}]',DOCUMENT_CUSTOM_FIELDS_RAW:'[{"field":4,"value":"ABC-123"}]',DOCUMENT_PAGE_COUNT:'4',DOCUMENT_MIME_TYPE:'application/pdf',DOCUMENT_ROOT_DOCUMENT:'',DOCUMENT_ROOT_DOCUMENT_ID:'',DOCUMENT_VERSIONS:'[]',DOCUMENT_METADATA_JSON:'{"id":42,"title":"Sample Service Agreement"}',DOCUMENT_RAW_JSON:'{"id":42,"title":"Sample Service Agreement","content":"Synthetic full document content for preview."}'};$('ragSourcePreview').textContent=renderTemplate($('ragSourceTemplate').value,values);$('ragSourcePreview').style.display='block'}

function previewAnswerTemplate(){const excerpts=renderTemplate($('ragSourceTemplate').value,{SOURCE_NUMBER:'1',CHUNK:'This is a synthetic retrieved passage for preview only.',DOCUMENT_ID:'42',DOCUMENT_TITLE:'Sample Service Agreement',DOCUMENT_CREATED:'2026-01-15',DOCUMENT_CORRESPONDENT:'Example Organization',DOCUMENT_TYPE:'Agreement'});$('ragAnswerPreview').textContent=renderTemplate($('ragAnswerTemplate').value,{DOCUMENT_EXCERPTS:excerpts,QUESTION:'What does the sample agreement say about renewal?',SEARCH_SCOPE:'all documents',SOURCE_COUNT:'1',CURRENT_DOCUMENT_ID:''});$('ragAnswerPreview').style.display='block'}
...
$('ragSaveBtn').onclick=saveRag;$('ragResetPromptBtn').onclick=()=>{$('ragSystemPrompt').value=ragDefaultSystemPrompt;markUnsaved('ragSaveStatus')};$('ragResetQueryTemplateBtn').onclick=()=>{$('ragQueryTemplate').value=ragDefaultQueryTemplate;markUnsaved('ragSaveStatus')};$('ragResetDocumentTemplateBtn').onclick=()=>{$('ragDocumentTemplate').value=ragDefaultDocumentTemplate;markUnsaved('ragSaveStatus')};$('ragResetSourceTemplateBtn').onclick=()=>{$('ragSourceTemplate').value=ragDefaultSourceTemplate;markUnsaved('ragSaveStatus')};$('ragResetAnswerTemplateBtn').onclick=()=>{$('ragAnswerTemplate').value=ragDefaultAnswerTemplate;markUnsaved('ragSaveStatus')};$('ragQueryPreviewBtn').onclick=previewQueryTemplate;$('ragDocumentPreviewBtn').onclick=previewDocumentTemplate;$('ragSourcePreviewBtn').onclick=previewSourceTemplate;$('ragAnswerPreviewBtn').onclick=previewAnswerTemplate;$('ragSyncBtn').onclick=()=>ragIndexAction('sync');$('ragRebuildBtn').onclick=()=>ragIndexAction('rebuild');$('ragPauseBtn').onclick=()=>ragIndexAction('pause');
for(const id of ['ragChatModel','ragSystemPrompt','ragSourceTemplate','ragAnswerTemplate','ragTimezone','ragThink','ragNumCtx','ragRetrievalTopK','ragTemperature','ragNumPredict','ragConversationHistory','ragRetrievalDiagnostics','ragSamplerTopK','ragTopP','ragMinP','ragRepeatPenalty','ragRepeatLastN','ragSeed','ragStop','ragEmbeddingModel','ragQueryTemplate','ragDocumentTemplate','ragEmbeddingDimensions','ragQueryTruncate','ragDocumentTruncate','ragEmbeddingNumCtx','ragChunkTarget','ragChunkOverlap','ragBatchSize','ragSliceSize','ragSyncInterval','ragHistoryMode','ragHistoryTurns','ragMinSimilarity','ragMaxChunksPerDoc','ragAdjacentChunks','ragRetrievalContextPercent']){$(id).addEventListener('input',()=>markUnsaved('ragSaveStatus'));$(id).addEventListener('change',()=>markUnsaved('ragSaveStatus'))}
...
def paperless_ui_setup_check(cfg):
    if not integration_package_ready():
        return {
            "ok": False,
            "reachable": False,
            "package_ready": False,
    ...
    try:
        with requests.get(
            cfg["connections"]["paperless_url"],
            headers={"Accept": "text/html"},
            timeout=5,
            stream=True,
        ) as result:
            result.raise_for_status()
            ready = result.headers.get("X-Paperless-Local-AI-UI") == "ready"
            if ready:
                return {
                    "ok": True,
                    "reachable": True,
                    "package_ready": True,
                    "detail": f"Paperless integration verified · HTTP {result.status_code}",
                }
            return {
                "ok": False,
                "reachable": True,
                "package_ready": True,
                "detail": (
                    "Paperless is reachable, but the UI integration is not loaded "
...
def tagging_state(*, force=False):
    ...
    matching = history_matching_settings(app_cfg, cfg["max_tags"])
    if force:
        history = refresh_history(max_tags=cfg["max_tags"], shutdown_after=False)
    else:
...
def preview_for(doc_id, config, *, shutdown_history_after=False):
    tax = client.taxonomy()
    doc = client.document(doc_id)
    tagging = history_context_for_document(
        config,
        doc,
        shutdown_after=shutdown_history_after,
    )
...
def finalize_model_result(result, tax, config, tagging, rendered):
    ...
    if not errors:
        matching = load_app_config()["correspondent_matching"]
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _dispatch(self):
        ...
        if self.command == "GET" and path == "/api/app/state":
            ...
            return response(
                ...
                HTTPStatus.OK,
                {
                    ...
                    "paperless_ui_integration_ready": integration_package_ready(),
                },
            )
        if self.command == "GET" and path == "/api/app/history":
            return response(self, HTTPStatus.OK, {"items": list_app_history()})
        if self.command == "GET" and path == "/api/app/paperless-ui/status":
            cfg = ensure_app_config()
            return response(self, HTTPStatus.OK, paperless_ui_setup_check(cfg))
        ...
        if self.command == "POST" and path == "/api/app/paperless-ui":
            payload = body_json(self)
            enabled = payload.get("enabled")
            if not isinstance(enabled, bool):
                raise ValueError("enabled must be true or false")
            if enabled and not Path("/integration").is_dir():
                raise ValueError("Paperless UI integration storage is not mounted in core-service; update the deployment before enabling the shortcut")
            cfg = load_app_config()
            if enabled:
                setup = paperless_ui_setup_check(cfg)
                if not setup["ok"]:
                    raise ValueError(
                        "Paperless shortcut setup is not ready: "
                        + setup.get("detail", "Paperless UI integration is not ready")
                    )
            cfg["paperless_ui"]["enabled"] = enabled
            if "control_center_url" in payload:
                cfg["paperless_ui"]["control_center_url"] = str(payload.get("control_center_url") or "")
            cfg = save_app_config(cfg, source="prompt-ui:paperless-ui")
            if Path("/integration").is_dir():
                sync_projection(cfg)
            return response(self, HTTPStatus.OK, {"ok": True, "config": cfg, "history": list_app_history(), "token_configured": bool(PAPERLESS_TOKEN), "paperless_ui_integration_ready": integration_package_ready()})
        ...
        if self.command == "POST" and path == "/api/app/correspondent-matching/test":
            payload = body_json(self)
            candidate = payload.get("candidate", "")
            if not isinstance(candidate, str):
                raise ValueError("candidate must be a string")
            matching = validate_correspondent_matching(payload.get("matching"))
            correspondents = [
                str(item["name"])
                for item in client.all_objects("/api/correspondents/")
                if item.get("name")
            ]
            ...
            return response(self, HTTPStatus.OK, {"simulation": simulation})
        ...
        if self.command == "POST" and path in {"/api/preview", "/api/test"}:
            ...
            tax, doc, tagging, rendered = preview_for(
                doc_id,
                cfg,
                shutdown_history_after=(path == "/api/test"),
            )
            base = {
