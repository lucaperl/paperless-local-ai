...
let currentRagConfig=null,currentRagState=null,ragDefaultSystemPrompt="",ragDefaultQueryTemplate="",ragDefaultDocumentTemplate="",ragDefaultAnswerTemplate="";
let ragQueryPlaceholders=[],ragDocumentPlaceholders=[],ragAnswerPlaceholders=[];
...
function ragDraft(){return {
  embedding_model:$('ragEmbeddingModel').value.trim(),embedding_query_template:$('ragQueryTemplate').value,document_embedding_template:$('ragDocumentTemplate').value,answer_prompt_template:$('ragAnswerTemplate').value,
...
function previewAnswerTemplate(){const excerpts='[Source 1] Document 42 — Sample Service Agreement\nThis is a synthetic retrieved passage for preview only.';$('ragAnswerPreview').textContent=renderTemplate($('ragAnswerTemplate').value,{DOCUMENT_EXCERPTS:excerpts,QUESTION:'What does the sample agreement say about renewal?',SEARCH_SCOPE:'all documents',SOURCE_COUNT:'1',CURRENT_DOCUMENT_ID:''});$('ragAnswerPreview').style.display='block'}
