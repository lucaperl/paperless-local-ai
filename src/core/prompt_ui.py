  ...
  <div class="section-grid">
    <div class="card section"><h2>Pipeline</h2><p>Paperless remains the document system of record. paperless-local-ai improves scanned-page OCR, then applies local metadata automation.</p>
      <div class="flow">
        ...
        <div class="flow-row local"><div class="flow-dot">3</div><div class="flow-copy"><div class="flow-title">Tag routing</div><div class="flow-desc">Hybrid tagging mode first checks reviewed Paperless documents for a high-confidence tag match. Otherwise the LLM decides tags; LLM direct mode always lets the model decide.</div></div></div>
  ...
  <div class="tab-page" id="class-tagging">
    ...
    <div class="card panel"><h3>Tagging strategy</h3><p class="mini">Choose how content tags are decided. Title, document type, date and sender still use the configured LLM.</p>
      <div class="strategy-grid">
        <label class="strategy-card"><div class="strategy-head"><div class="strategy-title"><input type="radio" name="taggingMode" value="history_assisted"><span>Hybrid tagging</span></div><span class="badge-rec">Recommended for small models</span></div><div class="strategy-copy">Compares documents with reviewed examples and reuses a tag only when similarity and neighbor agreement are strong. Otherwise the LLM decides using Tag Guidance and relevant examples.</div><div class="strategy-best"><strong>Best fit:</strong> compact local models, especially CPU-focused setups. · <a href="__TAGGING_DOCS_URL__#hybrid-tagging" target="_blank" rel="noreferrer">How Hybrid tagging works</a></div></label>
