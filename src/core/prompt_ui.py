...
<script>
let currentConfig=null,currentAppConfig=null,currentTaxonomy=[],currentHistoryStatus=null,paperlessUiReady=false;let classPromptPresets={};
...
function renderPaperlessUi(c){const s=c?.paperless_ui||{enabled:false,control_center_url:''},btn=$('appPaperlessUiToggleBtn'),status=$('appPaperlessUiStatus');btn.textContent=s.enabled?'Disable shortcut':'Enable shortcut';btn.className='btn '+(s.enabled?'':'good');status.textContent=s.enabled?`Enabled · opens ${s.control_center_url}`:(paperlessUiReady?'Disabled · integration package is ready.':'Disabled · integration files are not published yet.');status.className='status-box '+(s.enabled?'good':paperlessUiReady?'':'warn')}
...
async function loadApp(){try{const r=await api('/api/app/state');appFill(r.config,r.token_configured,r.paperless_ui_integration_ready);renderAppHistory(r.history||[]);checkConnections(r.config)}catch(e){setStatus('appSaveStatus',e.message,false)}}
