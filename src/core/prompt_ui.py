...
function correspondentMatchingDraft(){const similarity=Number($('appCorrespondentMinSimilarity').value)/100,margin=Number($('appCorrespondentMinMargin').value)/100;if(!Number.isFinite(similarity)||similarity<0.80||similarity>1)throw new Error('Minimum correspondent similarity must be between 80% and 100%.');if(!Number.isFinite(margin)||margin<0||margin>0.20)throw new Error('Minimum winner margin must be between 0 and 20 percentage points.');return {minimum_similarity:similarity,minimum_margin:margin}}
