...
- Normal metadata processing explicitly unloads configured Ollama models before leaving the AI transaction.
...
The public Control Center UI is English. Fresh-install OCR and prompt defaults are English, with English/German prompt presets available in the UI. Keep UI language, OCR language and prompt language independent.
...
Do not broaden compatibility claims without integration testing the target Paperless/OCRmyPDF release.
