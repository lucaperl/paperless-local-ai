...
- RAG endpoints on `core-service` require the internal Paperless relay secret. Browser access goes through authenticated/CSRF-protected `/_plai/` routes and is currently Paperless-superuser only. Do not expose the relay secret or `/data/rag` index files.
